...
def initialise_sql(host, user, password):
    try:
        ...
        return False
    if not os.path.exists(rf"{os.path.expanduser('~')}\Bookipedia"):
        os.makedirs(rf"{os.path.expanduser('~')}\Bookipedia")
    with open(rf"{os.path.expanduser('~')}\Bookipedia\creds.bin", 'wb') as f:
...
def connection():
    with open(rf"{os.path.expanduser('~')}\Bookipedia\creds.bin", 'rb') as f:
